...
export function ChatWindow({
  ...
  useEffect(() => {
    ...
    const ttlMs = Math.max(60, deviceAuth.expires_in) * 1000
    let delayMs = Math.max(3, deviceAuth.interval) * 1000
    ...
    <div className="flex flex-col flex-1 min-w-0 h-full overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            className="w-7 h-7 flex items-center justify-center rounded-lg transition-colors hover:bg-(--bg-hover)"
          ...
          {!gptAuthReady && (
            <button
              ...
              className="h-7 inline-flex items-center gap-1.5 px-2.5 rounded-lg text-[11px] font-medium border transition-colors hover:bg-(--bg-hover) disabled:opacity-60 disabled:cursor-not-allowed"
              style={{
          ...
          <a
            ...
            className="h-7 inline-flex items-center gap-1.5 px-2.5 rounded-lg text-[11px] font-medium border transition-colors hover:bg-(--bg-hover)"
            style={{
...
function CompanySelector() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border transition-all hover:bg-(--bg-hover)"
        style={{ borderColor: "var(--border-color)", color: "var(--text-secondary)" }}
      >
            ...
            >
              {allowedCompanies.map((c) => (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-colors hover:bg-(--bg-hover)"
                  style={{ color: "var(--text-primary)" }}
                >
